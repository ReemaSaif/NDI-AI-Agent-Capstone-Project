...
data_domains = [
    ("DG", "حوكمة البيانات", "إدارة البيانات", comp_map["إدارة البيانات"]),
    ("DQ", "جودة البيانات", "تحسين الجودة", comp_map["إدارة البيانات"]),
    ("DC", "تصنيف البيانات", "سياسات التصنيف", comp_map["الامتثال"]),
    ("PDP", "حماية البيانات الشخصية", "سياسات الحماية", comp_map["الامتثال"]),
]
...
for item in results:
    ...
    try:
        ...
        # CLASSIFICATION
        final_class = classification_map.get(
            item["dc"]["final_classification"],
            "Restricted"
        )

        cur.execute("""
            INSERT INTO data_classifications (classification_name)
            VALUES (%s)
            ON CONFLICT (classification_name)
            DO UPDATE SET classification_name = EXCLUDED.classification_name
        ...
        if finding_rows:
            execute_batch(cur, """
                ...
                (document_id, data_type, detected_value, risk_level)
                VALUES (%s, %s, %s, %s)
            """, finding_rows)
        ...
        """, (
            document_id,
            comp_status_map["Partially Compliant"],
            overall_score,
